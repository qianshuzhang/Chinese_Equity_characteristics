...
class AShareMarket:
    ...
    def __init__(self, mode):
        ...
        self.local_data_path = '/home/qianshu/Chinese_Factor/stock_database/new_tables/'
    ...
    def backtest(self, data, min_listed_time=6, backtest_start_year=2000, weight='e', port_num=5, plot=False,
                 title='None'):
        ...
        #
        df['Trdmnt'] = df.index
        df['Trdmnt'] = df['Trdmnt'].apply(str)
        df.set_index('Trdmnt', inplace=True)

        ret = self.monthly_ret.copy()
        listed = (~np.isnan(ret)).astype(int)
        listed = listed.cumsum()
        for i in range(ret.shape[1]):
            temp = np.array(listed.iloc[:, i])
            list_date = np.where(temp > 0)[0]
            if len(list_date) > 0:
                list_date = list_date[0]
                ret.iloc[list_date:np.min([list_date + min_listed_time, ret.shape[0]]), i] = np.nan
                df.iloc[list_date:np.min([list_date + min_listed_time, ret.shape[0]]), i] = np.nan
        ret = ret.shift(-1)

        size = self.get_data('TRD_Mnth', 'Msmvosd')

        # calc backtest
        backtest_money_ew = [[1] * port_num]
        backtest_money_vw = [[1] * port_num]
        backtest_ret_ew = []
        backtest_ret_vw = []
        backtest_firm_num = []
        backtest_time_list = []
        ...
        for i in range(df.shape[0] - 1):
            temp_df = np.array(df.iloc[i, :])
            temp_ret = np.array(ret.iloc[i, :])
            not_nan_num = np.sum(~np.isnan(temp_df))
            sort_result = np.argsort(temp_df)[:not_nan_num]
            port_lists = np.array_split(sort_result, port_num)
            ...
            temp_size = np.array(size.iloc[i, :])
            for port in range(port_num):
                port_list = port_lists[port]
                port_ret_ew = np.nanmean(temp_ret[port_list])
                port_rets_ew.append(port_ret_ew)
                # port_firm_nums.append(np.sum(~np.isnan(temp_ret[port_list])))

                port_raw_ret = temp_ret[port_list]
                port_size = temp_size[port_list]
                valid_list = ~np.isnan(port_raw_ret) & ~np.isnan(port_size)
                port_raw_ret = port_raw_ret[valid_list]
                port_size = port_size[valid_list]
                port_ret_vw = np.sum(port_raw_ret * port_size) / np.sum(port_size)
                port_rets_vw.append(port_ret_vw)
                port_firm_nums.append(len(port_raw_ret))

                port_df = temp_df[port_list]
                if port_df[valid_list].any():
                    bpsize = np.array(port_df[valid_list][0])
                    port_bp.append(bpsize)

            profit_ew = np.array(port_rets_ew)
            profit_vw = np.array(port_rets_vw)
            profit_ew[np.isnan(profit_ew)] = 0
            profit_vw[np.isnan(profit_vw)] = 0

            money_ew = np.array(backtest_money_ew[-1]) * (1 + profit_ew)
            money_vw = np.array(backtest_money_vw[-1]) * (1 + profit_vw)
            if int(df.index[i][:4]) >= backtest_start_year and int(df.index[i][:4]) < 2022:
                backtest_money_ew.append(money_ew)
                backtest_money_vw.append(money_vw)
                backtest_ret_ew.append(port_rets_ew)
                backtest_ret_vw.append(port_rets_vw)
                backtest_firm_num.append(port_firm_nums)
                backtest_time_list.append(df.index[i][0:4] + '-' + df.index[i][4:6])

                char_ew.append(port_rets_ew[-1] - port_rets_ew[0])
                char_vw.append(port_rets_vw[-1] - port_rets_vw[0])
                bp.append(port_bp)

        backtest_money_ew = np.delete(backtest_money_ew, 0, axis=0)
        backtest_money_vw = np.delete(backtest_money_vw, 0, axis=0)
        ...
        for i in range(0, df.shape[0] - 1):
            temp_ret = np.array(ret.iloc[i, :])
            temp_firm_num = np.sum(~np.isnan(temp_ret))

            temp_ret_ew = np.nanmean(temp_ret)

            temp_size = np.array(size.iloc[i, :])
            valid_list = ~np.isnan(temp_ret) & ~np.isnan(temp_size)
            temp_ret = temp_ret[valid_list]
            temp_size = temp_size[valid_list]
            temp_ret_vw = np.sum(temp_ret * temp_size) / np.sum(temp_size)

            if np.isnan(temp_ret_vw):
                money_ew = money_mkt_ew[-1]
                money_vw = money_mkt_vw[-1]
            else:
                money_ew = money_mkt_ew[-1] * (1 + temp_ret_ew)
                money_vw = money_mkt_vw[-1] * (1 + temp_ret_vw)
            if int(df.index[i][:4]) >= backtest_start_year and int(df.index[i][:4]) < 2022:
                money_mkt_ew.append(money_ew)
                money_mkt_vw.append(money_vw)
                ret_mkt_ew.append(temp_ret_ew)
                ret_mkt_vw.append(temp_ret_vw)
                firm_num_mkt.append(temp_firm_num)
        money_mkt_ew = np.delete(money_mkt_ew, 0)
        money_mkt_vw = np.delete(money_mkt_vw, 0)
        # print(money_mkt)

        ret_ew = np.array(backtest_ret_ew)
        ret_vw = np.array(backtest_ret_vw)
        #
        ret_mkt_ew = np.array(ret_mkt_ew)
        ret_mkt_vw = np.array(ret_mkt_vw)
        #
        cumret_ew = np.array(backtest_money_ew)
        cumret_vw = np.array(backtest_money_vw)
        #
        cumret_mkt_ew = np.array(money_mkt_ew)
        cumret_mkt_vw = np.array(money_mkt_vw)
        #

        firm_num = np.array(backtest_firm_num)  #
        firm_num_mkt = np.array(firm_num_mkt)
        firm_num_missing = firm_num_mkt - np.sum(backtest_firm_num, axis=1)

        # char_feature = pd.DataFrame({'Trdmnt':df['Trdmnt'],'char_feature':char})
        char_feature_ew = pd.DataFrame(char_ew, index=backtest_time_list)
        char_feature_vw = pd.DataFrame(char_vw, index=backtest_time_list)
        # char_feature.set_index('Trdmnt', inplace=True)
        bp = pd.DataFrame(bp)
        time_list = np.array(backtest_time_list)

        if plot:
            plt.figure(num=title, figsize=(40, 20), dpi=100)

        # ew ret
        if plot:
            ax1 = plt.subplot(331)
            for port in range(port_num):
                plt.plot(cumret_ew[:, port], label='port_' + str(port + 1))
            plt.plot(cumret_mkt_ew, label='market', ls='--')

            xtick_list = np.array(range(0, len(time_list), round(len(time_list) / 10)))
            # print(xtick_list)
            xtick_label = time_list[0:len(time_list):round(len(time_list) / 10)]
            # print(xtick_label)
            plt.tick_params(labelsize=12)
            plt.xticks(xtick_list, xtick_label)
            plt.legend()
            plt.title(title + ":equally cumulative return",fontsize=25)

        if plot:
            ax2 = plt.subplot(332)
            col_num = ret_ew.shape[1] + 1
            x = list(range(col_num))
            for i in range(len(x) - 1):
                x[i] = 'port_' + str(i + 1)
            x[-1] = 'market'
            y = list(np.nanmean(ret_ew, axis=0))
            y.append(np.nanmean(ret_mkt_ew))
            color_list = ['rosybrown', 'blueviolet', 'orange', 'green', 'red', 'purple', 'cyan', 'lemonchiffon',
                   'lightsteelblue', 'maroon', 'olive']
            plt.bar(x, y, color=color_list)
            plt.tick_params(labelsize=12)
            plt.title(title + ":equally average return",fontsize=25)

        if plot:
            ax3 = plt.subplot(333)
            for port in range(port_num):
                # start = np.where(firm_num_ew>0)[0][0]
                start = np.where(firm_num[:, port] > 0)[0][0]
                # plt.plot(firm_num_ew[start:, port], label='port_'+str(port+1))
                plt.plot(firm_num[start:, port], label='port_' + str(port + 1))
            # plt.plot(firm_num_missing_ew, label='missing', ls='--')
            plt.plot(firm_num_missing[start:], label='missing', ls='--')

            xtick_list = np.array(range(start, len(time_list), round(len(time_list) / 10)))
            xtick_label = time_list[start:len(time_list):round(len(time_list) / 10)]
            # print(xtick_list, xtick_label)

            plt.xticks(xtick_list, xtick_label)
            plt.legend()
            plt.title(title + ":firm numbers of 5 ports")

        if plot:
            ax6 = plt.subplot(336)
            # x = np.sum(firm_num_ew, axis=1)
            y = firm_num_mkt
            x = np.sum(firm_num, axis=1)
            x_major_locator = MultipleLocator(40)
            ax6.xaxis.set_major_locator(x_major_locator)
            plt.bar(time_list, y, label='market')

            plt.bar(time_list, x, label='char')
            plt.legend()
            plt.title(title + ":firm numbers")

        if plot:
            ax4 = plt.subplot(334)
            for port in range(port_num):
                plt.plot(cumret_vw[:, port], label='port_' + str(port + 1))
            plt.plot(cumret_mkt_vw, label='market', ls='--')
            ...
            plt.xticks(xtick_list, xtick_label)
            plt.legend()
            plt.title(title + ":value weighted cumulative return",fontsize=25)
        ...
        if plot:
            ...
            for i in range(len(x) - 1):
                x[i] = 'port_' + str(i + 1)
            x[-1] = 'market'
            y = list(np.nanmean(ret_vw, axis=0))
            y.append(np.nanmean(ret_mkt_vw))
            color_list = ['rosybrown', 'blueviolet', 'orange', 'green', 'red', 'purple', 'cyan', 'lemonchiffon',
                   'lightsteelblue', 'maroon', 'olive']
            plt.bar(x, y, color=color_list)
            plt.tick_params(labelsize=12)
            plt.title(title + ":value weightd average return",fontsize=25)

        rank = []
        raw = []
        for i in range(not_nan_num):
            rank.append((i / not_nan_num) * 2 - 1)
            raw.append(temp_df[sort_result[i]])
        # raw1 = winsorize(raw, limits=[0.005, 0.005])
        # raw2 = winsorize(raw, limits=[0.025, 0.025])
        ...
        if plot:
            ax7 = plt.subplot(337)
            plt.scatter(rank, raw1, s=0.05)
            plt.title(title + "_scaterplot of rank and raw data: winsorize 0.5%, 99.5%")

        if plot:
            ax8 = plt.subplot(338)
            plt.scatter(rank, raw2, s=0.05)
            plt.title(title + "_scaterplot of rank and raw data: winsorize 2.5%, 97.5%")

        if plot:
            ax9 = plt.subplot(339)
            label = ['20%', '40%', '60%', '80%']
            for port in range(1, 5):
                # plt.plot(bp_ew.iloc[:,port], label='ew_port_'+str(port)+ 'to'+str(port+1))
                plt.plot(bp.iloc[:, port], label=label[port - 1])

            xtick_list = np.array(range(0, len(time_list), round(len(time_list) / 10)))
            xtick_label = time_list[0:len(time_list):round(len(time_list) / 10)]
            plt.xticks(xtick_list, xtick_label)
            plt.legend()
            plt.title(title + ": breakpoint for 5 ports")

        return plt, char_feature_ew, char_feature_vw
...
# pd.set_option('max_row',380)
#mytest = AShareMarket('local')
#print(mytest.get_data('FS_Combas', 'A001101000'))
#print(len(chars_list_all))
#print(len(chars_list_quarter))
# to calculate
...
mytest = AShareMarket('local')
file_path = '/home/qianshu/Chinese_Factor/results1118'
#file_path = 'D:/Dropbox/Dropbox/csmar_chinese_factor/test result'
#mytest.rolling_ols_four_chbeta()
#print('rolling over')
for char in chars_list:
    function = getattr(mytest, 'calc_' + char)
    char_result = function()
    char_result.to_csv(file_path + '/chars/' + char + '.csv')
    plot, char_ew, char_vw = mytest.backtest(char_result, plot=True, title=char, backtest_start_year=2000,port_num=10)
    char_ew.to_csv(file_path + '/feature/' + char + '_ew_portfolio.csv')
    char_vw.to_csv(file_path + '/feature/' + char + '_vw_portfolio.csv')
    plot.savefig(file_path + '/plot/' + char + '.jpg', dpi=750)
